...
declare namespace vim {
  ...
  namespace lsp {
    ...
    const start: (opts: NvimLsp.ServerOptions) => number | undefined;
  ...
  namespace api {
    ...
    const nvim_create_autocmd: (
      event: string,
      options: {
        group?: string | number;
        pattern?: string | string[];
        buffer?: number;
        callback: string | ((args: AutocmdArgs) => boolean | void);
      }
    ) => number;
  ...
  namespace loop {
    ...
    const fs_stat: (filename: string) => { type: string } | undefined;
    const os_uname: () => { version: string };
  }
